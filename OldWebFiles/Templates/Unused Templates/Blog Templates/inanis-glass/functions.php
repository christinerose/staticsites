  ...
  echo ('<div style="position:fixed;bottom:33px;left:0;">');
    if(isset($baby)){
      foreach ($baby as $key => $value){
        ...
        ?>
        <div onmouseover="hovmhov();" onmouseout="unhovmhov();" class="mhov" id="hov<?php echo $matches[$key]; ?>">
        <div class="mframe">
          <?php
          if ($MenuOption==2){
          ...
          echo '<h4 style="text-align:center;">'.$parent.'</h4>';
          ?>
          <ul>
           <?php
...
function cats_insert (){
  ...
  foreach ($inputar as $key => $value){
    if ($key <> 0) {
      if($MenuKids<>0){
        $ct = $count + 1;
        $mover = ' onmouseover="mhov('.$matches[$count].','.$ct.')" ';
        $mout = 'onmouseout="munhov()"'.$inputar[$key];
      }
...
<div class="navf">
  <ul>
    <li onclick="molist(6);" style="cursor:default;line-height:10px;height:30px;width:25px;font-weight:bold;color:#fff;font-size:14px;">
        &nbsp;&nbsp;&raquo;&nbsp;&nbsp;
    </li>
  </ul>
</div><?php
...
function insert_molist(){
  global $molist;
  if ($molist!=""){
    echo '<div class="mhov" onclick="MOWasClicked=1;" id="SMSub6" style="position:fixed;bottom:33px;left:-200px;"><div class="mframe"><ul>';
    echo $molist;
    echo '</ul></div></div>';
  }
}
...
function insert_stylemenu(){
  ...
  <div class="SMRtOptsFl SMsh" onclick="FlyOutWasClicked=1;" id="SMSub5">
    ...
    <ul class="SMRtFlOpt">
      <li title="<?php _e('Empty, dark and mysterious.','inanis');?>" onclick="chooseStyle('none', 30)"><img class="switchbutton voidb" src="<?php bloginfo('template_directory'); ?>/images/void-button.png" alt="<?php _e('Void','inanis'); ?>" title="<?php _e('Void','inanis'); ?>" /><?php _e('Void','inanis'); ?> <?php if ($DefaultTheme==1){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
      <li title="<?php _e('The green bounty of life.','inanis');?>" onclick="chooseStyle('life-theme', 30)"><img class="switchbutton lifeb" src="<?php bloginfo('template_directory'); ?>/images/life-button.png" alt="<?php _e('Life','inanis'); ?>" title="<?php _e('Life','inanis'); ?>" /><?php _e('Life','inanis'); ?> <?php if ($DefaultTheme==2){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
      <li title="<?php _e('Subtle earth tones in browns and reds.','inanis');?>" onclick="chooseStyle('earth-theme', 30)"><img class="switchbutton earthb" src="<?php bloginfo('template_directory'); ?>/images/earth-button.png" alt="<?php _e('Earth','inanis'); ?>" title="<?php _e('Earth','inanis'); ?>" /><?php _e('Earth','inanis'); ?> <?php if ($DefaultTheme==3){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
      <li title="<?php _e('Bright and refreshing sky tones.','inanis');?>" onclick="chooseStyle('wind-theme', 30)"><img class="switchbutton windb" src="<?php bloginfo('template_directory'); ?>/images/wind-button.png" alt="<?php _e('Wind','inanis'); ?>" title="<?php _e('Wind','inanis'); ?>" /><?php _e('Wind','inanis'); ?> <?php if ($DefaultTheme==4){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
      <li title="<?php _e('The deep blue power of the ocean.','inanis');?>" onclick="chooseStyle('water-theme', 30)"><img class="switchbutton waterb" src="<?php bloginfo('template_directory'); ?>/images/water-button.png" alt="<?php _e('Water','inanis'); ?>" title="<?php _e('Water','inanis'); ?>" /><?php _e('Water','inanis'); ?> <?php if ($DefaultTheme==5){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
      <li title="<?php _e('Intense red-orange, raging inferno.','inanis');?>" onclick="chooseStyle('fire-theme', 30)"><img class="switchbutton fireb" src="<?php bloginfo('template_directory'); ?>/images/fire-button.png" alt="<?php _e('Fire','inanis'); ?>" title="<?php _e('Fire','inanis'); ?>" /><?php _e('Fire','inanis'); ?> <?php if ($DefaultTheme==6){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
      <li title="<?php _e('Light on color and on bandwidth.','inanis');?>" onclick="chooseStyle('lite-theme', 30)"><img class="switchbutton liteb" src="<?php bloginfo('template_directory'); ?>/images/1pxtrans.gif" alt="<?php _e('Light','inanis'); ?>" title="<?php _e('Light','inanis'); ?>" /><?php _e('Light','inanis'); ?> <?php if ($DefaultTheme==7){echo '<span style="font-size:9px">&laquo; '.__('Default','inanis').'</span>';} ?></li>
    </ul>
  </div><?php
}
